...
export function MyLearningPathPage() {
  ...
  function load() {
    setIsLoading(true);
    learningPathApi.mine().then(setData).finally(() => setIsLoading(false));
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <p className="text-xs font-medium uppercase tracking-wide text-accent-dark">My Learning Path</p>
      <h1 className="mt-1 font-display text-2xl text-charcoal sm:text-3xl">{data.journey.title}</h1>
      <p className="mt-1 text-sm text-charcoal-soft">Outcome: {data.journey.outcome}</p>

      <ProgressBar value={data.overallProgress} label="Path progress" className="mt-6" />

      {data.nextStep && (
        <div className="mt-6 flex flex-col gap-3 rounded-2xl border border-accent bg-accent-soft/40 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs text-accent-dark">Next recommended step</p>
            <p className="font-medium text-charcoal">{data.nextStep.title}</p>
          </div>
          <Button to={data.nextStep.type === 'course' ? `/courses/${data.nextStep.slug}` : `/projects/${data.nextStep.slug}`} size="sm">
            Continue Learning
          </Button>
        </div>
      )}
