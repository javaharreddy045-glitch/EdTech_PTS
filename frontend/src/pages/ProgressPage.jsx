...
export function ProgressPage() {
  ...
  const journeyProgress = data.currentJourney
    ? Math.round((data.currentJourney.completed_courses / data.currentJourney.total_courses) * 100)
    : 0;
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-border bg-white p-5">
          <p className="text-xs text-charcoal-soft">Current path</p>
          {data.currentJourney ? (
            <>
              <p className="mt-1.5 font-medium text-charcoal">{data.currentJourney.title}</p>
              <ProgressBar value={journeyProgress} className="mt-3" />
            </>
          ) : (
            <div className="mt-3">
              <p className="text-sm text-charcoal-soft">No active journey yet.</p>
              <Button to="/journeys" size="sm" className="mt-2">Browse Journeys</Button>
            </div>
          )}
        </div>
      </div>

      <div className="mt-6 rounded-2xl border border-accent bg-accent-soft/40 p-5">
        <p className="text-xs font-medium uppercase tracking-wide text-accent-dark">Next milestone</p>
        <p className="mt-1 text-sm text-charcoal">{data.nextMilestone || 'Follow a journey to see your next milestone.'}</p>
      </div>
        ...
        {data.recentActivity.length === 0 ? (
          ...
          <ul className="mt-4 flex flex-col gap-2">
            {data.recentActivity.map((activity, i) => (
              <li key={i} className="rounded-xl border border-border bg-white px-4 py-3 text-sm">
                <span className="capitalize text-charcoal-soft">{activity.kind}:</span>{' '}
                <span className="font-medium text-charcoal">{activity.label}</span>
              </li>
            ))}
          </ul>
        )}
